...
const NumResult = ({ movies1 }) => {
  return (
    <p className="num-results">
      Found <strong>{movies1.length}</strong> results
    </p>
  );
};
...
const MovieList = ({ movies }) => {
  ...
  return (
    <ul className="list">
      {movies?.map((movie) => (
        <ListMovie key={movie.imdbID} movie1={movie} />
      ))}
    </ul>
  );
};

const ListMovie = ({ movie1 }) => {
  return (
    <li key={movie1.imdbID}>
...
const ListWatchedMovie = ({ watched }) => {
  return (
    <ul className="list">
      {watched.map((movie) => (
        <li key={movie.imdbID}>
